...
class Orders
{
	...
	public function insert(array $items, string $orderId, array $settings = [])
	{
    	...
    	$discounts = $order = $bids = [];
    	
    	// Extract discounts
    	$dids = Core::normalizeArray(array_column($items, 'did'));
    	$query = $this->db->simple_select('bankpipe_discounts', 'bids, did', "did IN ('" . implode("','", $dids) . "')");
    	while ($discount = $this->db->fetch_array($query)) {
        	$discounts[$discount['did']] = Core::normalizeArray(explode(',', $discount['bids']));
    	}
    	
    	$appliedDiscounts = array_column($items, 'price', 'did');
    	unset($appliedDiscounts[0]);
    	...
    	foreach ($items as $key => $item) {
        	...
        	if ($discounts) {
            	...
            	foreach ($discounts as $did => $discount) {
                	...
                	if (in_array($item['bid'], $discount) or empty($discount)) {
                    	
                    	$dids[] = $did;
                    	$price += $appliedDiscounts[$did];
    		...
    		if ($settings['merchant']) {
        		...
        		$order[$key]['payee_email'] = $this->db->escape_string($settings['merchant']);
        		
        		$query = $this->db->simple_select(
    				'users',
    				'uid',
    				"payee = '" . $order[$key]['payee_email'] . "'"
    			);
	...
	{	
	    ...
	    if ($options['includeItemsInfo']) {
    	    ...
    	    foreach ($return as $invoice => $order) {
        	    
        	    foreach ($order['items'] as $key => $item) {
            	    
            	    $return[$invoice]['items'][$key]['originalPrice'] = $items[$item['bid']]['price'];
            	    
            	    if (is_array($items[$item['bid']])) {
            	        $return[$invoice]['items'][$key] += $items[$item['bid']];
                    }
