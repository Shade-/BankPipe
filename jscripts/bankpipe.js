					...
					// Success, open window
						...
						promise.then(response => {
							...
							// Success
								...
								$.jGrowl(BankPipe.lang.paymentSuccessful.replace('{reference}', response.reference));
								...
								if (response.invoice) {
    								...
    								setTimeout(() => {
        								...
        								window.location.href = './usercp.php?action=purchases&invoice=' + response.invoice;
							...
							// Pending
								...
								return $.jGrowl(BankPipe.lang.paymentPending.replace('{reference}', response.reference));
							}
			...
			$('.bankpipe-item').on('click', function(e) {
				...
				var callback = function(response) {
					...
					if (response.message) {
						...
						var addClass = 'uk-button-primary';
						var removeClass = 'uk-button-danger';
						...
						if (itemsCount > 0) {
							cartNav.removeClass('uk-hidden').text(itemsCount);
						}
						else {
